...
export default function Receipt() {
    ...
    const { data: order, isLoading, error } = useQuery({
        ...
        queryFn: async () => {
            ...
            return response.data.order // API returns {order: {...}}
        },
        enabled: !!id,
        retry: false,
    })
    ...
    const receiptData: ReceiptData = {
        ...
        taxRate: 0.08,
        taxAmount: (order.totalAmount || 0) * 0.08,
        shipping: 0,
        grandTotal: (order.totalAmount || 0) * 1.08,
        ...
        <div className="min-h-screen bg-[#f4f4f5] p-4 md:p-8 font-sans text-black overflow-x-hidden print:bg-white print:p-0">
            ...
            <div className="max-w-4xl mx-auto flex justify-between items-center mb-8 sticky top-4 z-50 print:hidden">
                ...
                <div className="flex gap-4">
                    <BrutalistButton variant="primary" icon={<Printer size={18} />} onClick={() => window.print()}>
                        Print Receipt
                    </BrutalistButton>
                    <BrutalistButton variant="secondary" icon={<Download size={18} />} onClick={() => window.print()}>
                        Save as PDF
                    </BrutalistButton>
                </div>
            </div>
